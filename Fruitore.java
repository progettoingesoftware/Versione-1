...
public class Fruitore extends Utente implements Serializable {
	...
	public Fruitore(String n, String c, int an, int mn, int gn, String u, String p) {
		...
		/**
		 * L'attributo dataDiIscrizione assume il valore restituito dal metodo now()
		 * della classe LocalDate; tale valore e' costituito dalla data in cui viene
		 * effettuata tale invocazione ottenuta attraverso l'orologio di sistema
		 */
		this.dataDiIscrizione = LocalDate.now();

		/**
		 * L'attributo dataDiScadenza assume il valore indicato dalla data di iscrizione
		 * incrementata di un periodo di 5 anni
		 */
	...
	/**
	 * La verifica della data avviene mediante due if concatenati: il primo verifica
	 * che la data corrente preceda quella di scadenza indicata per lo specifico
	 * fruitore; il secondo verifica che la data corrente succeda quella di scadenza
	 * (indicata per lo specifico fruitore) diminuita di un periodo di 10 giorni
	 * 
	...
	/**
	 * Metodo toString() ereditato dalla classe String per la creazione di una
	 * stringa descrittiva contenente i vari attributi dell'oggetto Fruitore
	 * 
	...
	public String toString() {
		...
		ris.append(String.format(DESCRIZIONE_FRUITORE, getNome(), getCognome(), getUsername(), getPassword(),
				dataDiNascita.format(formatter), dataDiIscrizione.format(formatter), dataDiScadenza.format(formatter)));
		return ris.toString();
	}
